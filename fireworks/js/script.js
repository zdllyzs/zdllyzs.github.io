...
function seqCPY(){
    ...
    const offset = Math.random() * 0.08 - 0.04;
    const shell1 = new Shell(shellType(baseSize));
    shell1.launch(0.5 + offset, 0.7, COLOR.Blue, -1, -1);

    const delay1 = 1000 + Math.random() * 400;
    const delay2 = 3000 + Math.random() * 400;
    const delay3 = 6000 + Math.random() * 400;

    setTimeout(() => {
        const offset = Math.random() * 0.08 - 0.04;
        const shell2 = new Shell(shellType(smallSize));
        shell2.launch(0.2 + offset, 0.1, COLOR.Pink, -1, 0);
    }, delay1);

    setTimeout(() => {
        const offset = Math.random() * 0.08 - 0.04;
        const shell3 = new Shell(shellType(smallSize));
        shell3.launch(0.5 + offset, 0.1, COLOR.Gold, 0, -1);
    }, delay2);

    setTimeout(() => {
        const offset = Math.random() * 0.08 - 0.04;
        const shell4 = new Shell(shellType(smallSize));
        shell4.launch(0.8 + offset, 0.1, COLOR.Red, 1, -1);
    }, delay3);
...
//获取图片点阵信息
function getImgTemplateDots(picSeq) {
    var img = (picSeq == undefined || picSeq == null) ? imageTemplateManager.randomSource() : imageTemplateManager.sources[picSeq];
    if (!img) return null;
 ...
 */
function createWordBurst(wordText, particleFactory, center_x, center_y, color) {
    //将点阵坐标转换为canvas坐标
...
function createImgBurst(particleFactory, center_x, center_y, picSeq, color) {
    var map = getImgTemplateDots(picSeq);
    if (!map) return;
    var dcenterX = map.width / 2;
    var dcenterY = map.height / 2;
    var color = color || randomColor();
...
class Shell {
     ...
     */
    launch(position, launchHeight, scolor, wordSeq, picSeq) {
        this.color = scolor || this.color;
        this.glitterColor = scolor || this.glitterColor;
        ...
        //爆炸回调
        comet.onDeath = comet => this.burst(comet.x, comet.y, this.color);
    ...
    burst(x, y, scolor) {
        ...
        if (!this.disableWord && store.state.config.wordShell) {
            if (Math.random() < 1) {
                if(this.wordSeq != undefined && this.wordSeq != null && this.wordSeq != ''){
                    createWordBurst(randomWords[this.wordSeq], dotStarFactory, x, y, scolor);
                }else{
                    if(this.wordSeq != -1){
                ...
                if(this.picSeq != undefined && this.picSeq != null && this.picSeq != ''){
                    createImgBurst(dotStarFactory, x, y, this.picSeq, scolor);
                }else{
                    if(this.picSeq != -1){
